...
def history():
    ...
    return render_template('tarefas/history.html', tasks=tasks)
...
def reject_task(id):
    ...
    if task is None:
        ...
        db.execute(
            "UPDATE tarefa SET status = 'rejeitada', updated_at = datetime('now', '-3 hours') WHERE id = ?",
            (id,)
        )
        db.commit()
        flash('Tarefa rejeitada.')
